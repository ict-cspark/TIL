...
for test_case in range(1, T + 1):
    ...
    result = 'Possible'             # result 기본값을 possible로 초기화
    for i in range(N):              # 사람수만큼 반복문 실행
        bread = (people[i] // M) * K    # 손님이 온 시간에서 붕어빵 만드는 시간을 나눠 몫을 구한 후 한번에 만드는 붕어빵 개수를 곱한다.
        if bread <= i:              # 빵갯수가 손님수보다 적으면
            result = 'Impossible'   # result를 impossible 로 변경 후 break
            break
