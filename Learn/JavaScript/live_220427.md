...
+ 둘 다 Live Collection으로 DOM의 변경사항을 실시간으로 반영하지만,
  ...
  querySelectorAll()에 의해 반환되는 NodeList는 Static Collection으로 실시간으로 반영되지않음
...
+ Static Collection (non-live)
  ...
  + querySelectorAll()의 반환 NodeList만 statitc collection
...
+ Node.appendChild()
  ...
  + 만약 주어진 Node가 이미 문서에 존재하는 다른 Node를 참조한다면 새로운 위치로 이옫ㅇ
...
+ 즉, 줄 바꿈을 인식하고 숨겨진 내용을 무시하는 등 최종적으로 스타일이 적용된 모습으로 표현
...
+ `EventTarget.addEventListener`
  + 지정한 이벤트가 대상에 전달될 때마다 호출될 함수를 설정
...
#### 'EventTarget(대상).addEventListener(type(특정 이벤트), listener(할 일))'
